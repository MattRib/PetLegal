...
<section class="my-5 container text-center">
    ...
    $categ = "SELECT DISTINCT categoria FROM artigo";
    $c = mysqli_query($conn, $categ);
    
    $pri = $categoria[1] = mysqli_fetch_assoc($c);
    $seg = $categoria[2] = mysqli_fetch_assoc($c);
    
    print_r($pri);
    print_r($seg);
    
    $s = "SELECT * FROM `artigo` WHERE  data <= NOW() AND categoria = %'$pri'% AND status = '1' LIMIT 1";
    $va = mysqli_query($conn, $s);

    $j = "SELECT * FROM `artigo` WHERE  data <= NOW() AND categoria ='$seg' AND status = '1' LIMIT 1";
    $vj = mysqli_query($conn, $j);
    ...
    while ($r = mysqli_fetch_assoc($va)) {
        ...
        <div class="row">
            ...
            <div class="col-lg-5">
                ...
                <div class="view overlay rounded z-depth-2 mb-lg-0 mb-4">
                    <img class="img-fluid" src="img_art/<?= $r['foto']; ?>" alt="Sample image">
                    <a>
            ...
            <div class="col-lg-7">
                ...
                <!-- Read more button -->
                <a class="btn btn-primary btn-md" href="area_art.php?content=<?= $r['id_artigo']; ?>">Ler Mais</a>
        ...
        <hr class="my-5">
    <?php } ?>
    <?php while ($p = mysqli_fetch_assoc($vj)) { ?>
        <!-- Grid row -->
        ...
        <div class="row">
            ...
            <div class="col-lg-7">
                ...
                <a href="#!" class="indigo-text">
                    <h6 class="font-weight-bold mb-3">
                        <i class="fa fa-star pr-2"></i><?php echo $p['categoria'] ?></h6>
                </a>
                <!-- Post title -->
                <h3 class="font-weight-bold mb-3">
                ...
                <p><?= $p['resumo']; ?></p>
                <!-- Post data -->
                <p>por <a><strong><?= $p['autor']; ?></strong></a>, <?php echo date("d/m/Y", strtotime($p['data'])); ?></p>
                <!-- Read more button -->
                <a class="btn btn-primary btn-md" href="area_art.php?content=<?= $p['id_artigo']; ?>">Ler Mais</a>
            ...
            <div class="col-lg-5">
                ...
                <!-- Featured image -->
                <div class="view overlay rounded z-depth-2 mb-lg-0 mb-4">
                    <img class="img-fluid" src="img_art/<?= $p['foto']; ?>" alt="Sample image">
                    <a>
                        <div class="mask rgba-white-slight"></div>
                    </a>
                </div>
